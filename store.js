...
// ComicVine populates no team credits on a volume -- verified against the
// Uncanny X-Men Omnibus, which reports 357 characters and zero teams. So a
// team's books are derived from its line-up instead: volumes that credit its
// members, ranked by how many of them appear. Still real saved credits, never a
// keyword guess, but it is an inference and the UI must say so.
const selectVolumesByCharacter = db.prepare(`SELECT v.payload, v.fetched_at, l.to_id AS member
  FROM catalogue_links l
  JOIN catalogue_volumes v ON v.id = l.from_id
  WHERE l.from_kind = 'volume' AND l.to_kind = 'character'
    AND l.to_id IN (SELECT value FROM json_each(?))
  ORDER BY v.fetched_at DESC`);

export function volumesForCharacters(ids, limit = 48) {
  const list = (Array.isArray(ids) ? ids : []).map(String).filter(Boolean);
  if (!list.length) return [];
  const cap = Math.min(96, Math.max(1, Number(limit) || 48));
  const volumes = new Map();
  const byMember = new Map(list.map((id) => [id, []]));
  for (const row of selectVolumesByCharacter.all(JSON.stringify(list))) {
    let volume;
    try { volume = JSON.parse(row.payload); } catch { continue; }
    const id = String(volume.id);
    if (!volumes.has(id)) volumes.set(id, { volume, members: new Set(), fetchedAt: Number(row.fetched_at) || 0 });
    const record = volumes.get(id);
    record.members.add(String(row.member));
    byMember.get(String(row.member))?.push(id);
  }

  // A team shelf needs two kinds of books: a compact opening run where several
  // actual line-up members are credited, then a round-robin of the members'
  // own books. A simple global "most members" sort turned 48 slots into
  // Wolverine volumes because every Wolverine book happened to cache a large
  // guest cast from ComicVine.
  const sharedCap = Math.min(Math.ceil(cap / 3), 16);
  const shared = [...volumes.values()]
    .filter((record) => record.members.size > 1)
    .sort((a, b) => b.members.size - a.members.size || b.fetchedAt - a.fetchedAt);
  const chosen = [];
  const seen = new Set();
  const add = (record) => {
    const id = String(record.volume.id);
    if (seen.has(id) || chosen.length >= cap) return false;
    seen.add(id);
    chosen.push({ ...record.volume, memberHits: record.members.size });
    return true;
  };
  for (const record of shared.slice(0, sharedCap)) add(record);

  // Give every member with locally saved credits a turn before any member gets
  // a second. The roster order came from Wikidata's popularity ranking, so the
  // first pass is a familiar introduction rather than alphabetical noise.
  let progress = true;
  const cursors = new Map(list.map((id) => [id, 0]));
  while (chosen.length < cap && progress) {
    progress = false;
    for (const member of list) {
      const candidates = byMember.get(member) || [];
      let cursor = cursors.get(member) || 0;
      while (cursor < candidates.length && seen.has(candidates[cursor])) cursor += 1;
      cursors.set(member, cursor + 1);
      if (cursor < candidates.length && add(volumes.get(candidates[cursor]))) progress = true;
